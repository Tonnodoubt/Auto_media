...
4. **当前代码不修改时，要尽量完整可用，必须由本机统一入口 `8080` 同时承接前端页面、API、`/health` 和 `/media`。**
...
当前仓库的代码现状决定了“前后端分开暴露”不适合作为试用方案，原因已经能从代码里直接确认：

1. 后端当前 `CORS` 只允许 `http://localhost:5173`  
   见 `app/main.py`

2. 前端有部分逻辑会回落到 `http://localhost:8000`  
   见 `frontend/src/views/VideoGeneration.vue`  
   见 `frontend/src/components/CharacterDesign.vue`

3. 设置页的 `backendUrl` 保存在浏览器本地 `localStorage`，不是服务端统一配置  
   见 `frontend/src/stores/settings.js`

4. 当前前端仍有 Mock 逻辑  
   `MOCK_ENABLED = true`，`useMock = MOCK_ENABLED && !llmApiKey`  
   见 `frontend/src/stores/settings.js`

5. FastAPI 根路径 `/` 现在返回的是 API 说明 JSON，不是前端页面  
   见 `app/main.py`
...
### 4.1 `backendUrl` 对完整试用应视为必填

当前代码下，首次在试用人员浏览器中打开页面后，应在设置页把：

```text
backendUrl = http://<ECS公网IP>:18080
```

原因：

- 这是浏览器本地设置，不是服务端全局配置
- 换浏览器、换设备、无痕窗口重开，都可能需要重新设置
- 不设置时，部分页面仍可能走错到 `localhost`
- 其中角色图相关页面在未设置时会直接回落到 `http://localhost:8000`，因此如果目标是“尽量完整试用”，这里不应只当成可选建议
- 当前设置页输入框 placeholder 仍写着“留空使用默认 `http://localhost:8000`”，这是开发遗留提示，公网试用时不要按这个文案理解
...
- `MOCK_ENABLED = true`
- `useMock` 取决于浏览器本地是否存在 `llmApiKey`

这意味着：

- 即使服务端 `.env` 已经配好默认 LLM Key
- 试用人员自己的浏览器如果没有本地 `llmApiKey`
- 某些流程仍可能进入前端 Mock 分支，而不是真实调用

如果你的目标是“尽量完整试用真实功能”，必须额外核对这一点。
...
2. 不需要访问 `/docs`
3. 不需要访问任何单独后端端口
4. 首次进入后，在设置页把 `backendUrl` 明确填成 `http://<ECS公网IP>:18080`
...
- 设置页里出现的 `localhost:8000` placeholder 不应被当成公网试用配置
...
| 现象 | 优先排查 |
|------|---------|
| 首页能开，但角色图加载失败 | `backendUrl` 是否未填或仍回落到 `localhost` |
| 视频页能进，但生成/轮询失败 | Mock 模式、`backendUrl`、统一入口代理规则 |
